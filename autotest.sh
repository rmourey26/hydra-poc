...
SPECS=$(git ls-files  'hydra-node/**/*Spec.hs'  'hydra-plutus/**/*Spec.hs'  'hydra-model/**/*Spec.hs' | \
          sed -e 's/^[^A-Z]*\(.*\)\.hs$/\1/' | sed 'y=/=.=' | sed -e 's/$/.spec/' | tr -s '\012' ',' | sed -e 's/,$//' )

COMMAND="cabal exec ghci -- -ihydra-plutus/src -ihydra-plutus/test -ihydra-model/src -ihydra-model/test -ihydra-node/src -ihydra-node/test $FLAGS $(git ls-files 'hydra-node/**/*.hs'  'hydra-plutus/**/*.hs' 'hydra-model/**/*.hs' | grep -v Main.hs| grep -v Repl | tr -s '\012' ' ')"
